...
contains
  ...
  subroutine create_flow_chan
    ...
    if (IC_type == 0) then
      ...
      ! Thermocline-isolated front
      ! NOTE: Parameters defined in create_th_chan
      do j = 0, Nyp
        do k = 0, Nzp - 1
          do i = 0, Nxm1
            u1(i, k, j) = 0.d0
            ...
            u3(i, k, j) = -2.d0 * delta / Lx * (gyf(j) - 0.5 * Ly) & ! Remove the TWS from solver
                            + 0.5d0 * ( &
                                gyf(j) * (Gamma_bot/Gamma + 1.d0 / (cosh((gx(i) - 0.5 * Lx) / delta)**2.d0 &
                                                                    * tanh(0.5*Lx/delta))   )  &
                                - Htrans * (Gamma_bot/Gamma - 1.d0 / (cosh((gx(i) - 0.5 * Lx) / delta)**2.d0 &
                                                                    * tanh(0.5*Lx/delta))   ) &
                                     * (log(cosh((1.0-H_top-gyf(j))/Htrans)) - log(cosh((1.0-H_top)/Htrans))) &
                                ) &   ! Layered structure
                            + zeta_eddy * (gx(i) - 0.5 * Lx) &
                                + (-zeta_eddy * 0.5 * Lx * tanh((gx(i) - 0.5 * Lx) / delta)) & ! Barotropic Flow
                                      / tanh(0.5*Lx/delta) ! (Make BT exactly periodic)
    ...
    do n = 1, N_th
      if (create_new_th(n)) then
        ...
        if (IC_type == 0) then
          ...
          ! Thermocline-isolated front
          Gamma_bot = 2.d0 * delta * Gamma / Lx
          H_top = 0.4d0
          Htrans = 0.1d0
          N2f2_top = 1.d3
          N2f2_bot = 5.d5
          zeta_eddy = -0.25d0 / Gamma
          ...
          do k = 0, Nzp - 1
            do i = 0, Nxm1
              do j = 1, Nyp
                th(i, k, j, n) = &
                                  0.5d0 * (1.0 - tanh((gyf(j) - (1.0 - H_top)) / Htrans)) &
                                    * (Gamma_bot / Gamma**2 * (gx(i) - 0.5 * Lx  - Lx/3.d0) &  ! Bottom M^2 bit
                                      + N2f2_bot / Gamma**2 * (gyf(j) - (1.0 - H_top)) ) &  ! Bottom N^2 bit
                                + 0.5d0 * (1.0 + tanh((gyf(j) - (1.0 - H_top)) / Htrans)) &
                                    * (2.d0 * Ro_inv * 0.5d0 * tanh((gx(i) - 0.5 * Lx) / delta) &
                                          / tanh(0.5d0 * Lx / delta) &                      ! Top M^2 bit
                                      + N2f2_top / Gamma**2 * (gyf(j) - (1.0 - H_top)) ) &  ! Top N^2 bit
                                + Ro_inv * (1.d0 - 2.d0 / Lx * gx(i)) ! Remove BG Linear M^2
