module parameters
  ...
  ! Rotating Flows
  real(rkind) Ro_inv, grav_x, grav_y, grav_z, delta, Gamma

  ! Parameters for oscillatory forcing
  real(rkind) omega0, amp_omega0, force_start
  real(rkind) w_BC_Ymax_c1_transient

  ! Parameters for Isolated TXLA Front
  real(rkind) Gamma_bot, H_top, N2f2_top, N2f2_bot, Htrans, zeta_eddy
...
contains
  ...
  subroutine input_chan
    ...
    else if (IC_Type == 6 .or. IC_Type == 7 .or. IC_Type == 8 .or. IC_Type == 9) then ! Finite Front
      if (w_BC_Ymin == 1) then
    ...
    if (IC_Type == 5 .or. IC_Type == 6 .or. IC_Type == 7 .or. IC_Type == 8 .or. IC_Type == 9) then
      homogeneousX = .false.
    else ! Infinite, homogeneous front (or other IC...)
      homogeneousX = .false. ! Assume the x-direction is a valid averaging dimension
    endif
