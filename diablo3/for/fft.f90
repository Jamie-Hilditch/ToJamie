...
contains
  ...
  subroutine init_fft(u1,cu1)
    !----*|--.---------.---------.---------.---------.---------.---------.-|-------|
    real(C_DOUBLE), pointer, contiguous :: u1(:,:,:) !(0:Nx+1,0:Nzp+1,0:Nyp+1)
    complex(C_DOUBLE_COMPLEX), pointer, contiguous :: cu1(:,:,:) !(0:Nxp,0:Nz+1,0:Nyp+1)
    integer i, j, k
    ...
    disp_mem = 16
    call mpi_win_create(temp_fft, temp_fft_win_size, &
  ...
  subroutine fft_xz_to_fourier(v, vv)
    ...
    real(rkind), contiguous, pointer :: v(:,:,:) !(0:Nx + 1, 0:Nzp + 1, 0:Nyp + 1)
    complex(rkind), contiguous, pointer :: vv(:,:,:) !(0:Nxp, 0:Nz + 1, 0:Nyp + 1)
